...
///     options.
/// </summary>
public abstract partial class CommandPopupAutocomplete : AutocompleteBase
{
    public bool IsSuggestionSelecting { get; protected set; }
    ...
    /// <summary>Creates a new instance of the <see cref="CommandPopupAutocomplete"/> class.</summary>
    public CommandPopupAutocomplete()
    {
        SuggestionGenerator = new ArgumentSuggestionGenerator();
    ...
    public override void RenderOverlay(Point renderAt)
    {
        ...
        Suggestion[] toRender = [.. Suggestions.Skip(ScrollOffset).Take(height)];
    ...
    {
        // TODO: Sloppy hope Terminal.Gui.TextView will have some way to insert a full text and trigger text changed event
        IsSuggestionSelecting = true;
        ...
        IsSuggestionSelecting = false;

        return true;
    }

    /// <summary>Get the text in the <see cref="HostControl"/>.</summary>
    ...
    protected bool Select()
    {
        if (SelectedIdx >= 0 && SelectedIdx < Suggestions.Count)
        {
            ...
            if (SuggestionGenerator is ArgumentSuggestionGenerator argGen)
            {
                if (/*!argGen.IsCurrentWordIsSuggestion(GetText(), GetCursorPosition(), accepted) && */InsertSelection(accepted))
                {
                    argGen.ChangeSelectedIdx(SelectedIdx);
                    return true;
                }
                return false;
            }
    ...
    /// <summary>Get the cursor position in the <see cref="HostControl"/>.</summary>
